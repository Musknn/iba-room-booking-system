...
const BreakoutBooking = ({ studentERP }) => {
  ...
  const checkAvailability = async () => {
    ...
    try {
      ...
      const response = await fetch(
        ...
        `date=${filters.date}&startTime=${selectedSlot.start}&endTime=${selectedSlot.end}&buildingId=${filters.buildingId}&roomType=Breakout`
      );
          ...
          {availableRooms.length > 0 ? (
            <div className="breakout-rooms-list">
              {availableRooms.map(room => (
                <div key={room.ROOM_ID} className="breakout-room-card">
            ...
            <div style={{ 
              ...
              {loading ? 'Searching for available breakout rooms...' : 'No breakout rooms available for selected criteria'}
            </div>
          )}
        </div>
